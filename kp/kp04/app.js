...
function renderSimulation(mission) {
  if (mission.id === 4) {
    ...
    const situations = bm
      ? [
          ["Buka skru casing", "screwdriver", "Gunakan pemutar skru Phillips yang sesuai supaya kepala skru tidak rosak."],
          ["Pegang RAM / motherboard", "esd", "Gunakan anti-static wrist strap untuk kurangkan risiko ESD."],
          ["Skru kecil mudah hilang", "tray", "Gunakan screw tray untuk asing dan simpan skru."],
          ["CPU perlu cooler", "paste", "Letak thermal paste sederhana antara CPU dan cooler."],
          ["Kabel menghalang airflow", "tie", "Gunakan cable tie atau velcro untuk kemas kabel."],
          ["Habuk pada casing/fan", "brush", "Gunakan berus lembut atau blower dengan berhati-hati."]
        ]
      : [
          ["Open casing screws", "screwdriver", "Use the correct Phillips screwdriver so the screw head is not damaged."],
          ["Handle RAM / motherboard", "esd", "Use an anti-static wrist strap to reduce ESD risk."],
          ["Small screws can be lost", "tray", "Use a screw tray to separate and store screws."],
          ["CPU needs cooler", "paste", "Apply moderate thermal paste between CPU and cooler."],
          ["Cables block airflow", "tie", "Use cable ties or velcro to organise cables."],
          ["Dust on casing/fan", "brush", "Use a soft brush or blower carefully."]
        ];
    const tools = bm
      ? [
    ...
    return `
      <div class="kp04-main-sim">
        <div class="sim-instruction">
          ${bm ? "Pilih satu situasi, kemudian pilih tool yang betul." : "Select a situation, then choose the correct tool."}
        </div>
        <div class="situation-list">
          ${situations.map(([title, answer, info], index) => `
            <button class="situation-chip" data-kp04-main-situation="${title}" data-main-answer="${answer}" data-main-info="${info}" style="--delay:${index}">
              ${title}
            </button>
          `).join("")}
        </div>
        <div class="tool-choice-bank">
          ${tools.map(([value, label]) => `<button class="btn" data-kp04-main-tool="${value}">${label}</button>`).join("")}
        </div>
        <div id="kp04MainResult" class="hotspot-result">
          ${bm ? "Belum pilih situasi." : "No situation selected yet."}
        </div>
      </div>
    `;
  }
...
function bindKp04Game() {
  ...
  document.querySelectorAll("[data-kp04-main-situation]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      document.querySelectorAll("[data-kp04-main-tool]").forEach((item) => item.classList.remove("selected"));
      button.classList.add("active");
      const result = document.getElementById("kp04MainResult");
      result.innerHTML = `<strong>${selectedKp04Situation.title}</strong><span>${state.lang === "bm" ? "Sekarang pilih tool yang sesuai." : "Now choose the suitable tool."}</span>`;
    });
  });
  ...
  document.querySelectorAll("[data-kp04-main-tool]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (!selectedKp04Situation) {
        result.innerHTML = `<strong>${state.lang === "bm" ? "Pilih situasi dahulu" : "Select a situation first"}</strong><span>${state.lang === "bm" ? "Klik situasi kerja di atas." : "Click a work situation above."}</span>`;
        return;
      }

      const correct = button.dataset.kp04MainTool === selectedKp04Situation.answer;
      result.classList.toggle("correct", correct);
      result.classList.toggle("incorrect", !correct);
      result.innerHTML = `
        <strong>${correct ? (state.lang === "bm" ? "Betul" : "Correct") : (state.lang === "bm" ? "Belum tepat" : "Not yet")}: ${selectedKp04Situation.title}</strong>
        <span>${correct ? selectedKp04Situation.info : (state.lang === "bm" ? "Cuba pilih tool lain." : "Try another tool.")}</span>
      `;
    });
  });
